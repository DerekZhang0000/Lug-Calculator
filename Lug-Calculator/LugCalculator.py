...
def loadGUI():  # loads tkinter GUI
    ...
    window.iconbitmap(True, "lug.ico")
    ...
    def axialLoadGUI(default=False):    # initializes input fields for axial load
        ...
        yieldBushingBearingCompressiveStrength = ttk.Entry(name="skipCheck")
        ...
        if default:
            ...
            fos.insert(0, 1.5)
            yieldBushingBearingCompressiveStrength.insert(0, "")
    ...
    def allValidComplete():  # returns whether or not all fields are filled out and valid
        ...
        for inputField in inputFields[0][:int(len(inputFields[0]) / 2)]:
            if str(inputField).split(".")[-1] == "skipCheck" and inputField.get() == "":
                continue
    ...
    def enter(): # submits the data you enter
        if allValidComplete():
            if loadType.get() == "Axial":
                ...
                inputDataList = []
                hD, pD, eD, w, t, tUS, tYS, eM, mS, dL, fos = [float(element.get()) for element in loadElements[:7] + loadElements[8:]]
                if loadElements[7].get() == "":
                    yBBCS = "N/A"
                else:
                    yBBCS = float(loadElements[7].get())
                inputDataList = [hD, pD, eD, w, t, tUS, yBBCS, tYS, eM, mS, dL, fos]
...
def axialLoad(holeDiameter, pinDiameter, edgeDistance, width, thickness, tensileUltimateStrength, tensileYieldStrength, yieldBushingBearingCompressiveStrength, elasticModulus, maxStrainAtBreakage, designLoad, factorOfSafety):
    ...
    try:
        ...
        ultimateBushingBearingLoad = "N/A"
    ...
    subPlot1.set_title("K Factor Graphs")
    subPlot1.set_xlabel("Hole Diameter / Width")
    subPlot1.set_ylabel("Stress Coefficient")
    ...
    if yieldBushingBearingCompressiveStrength == "N/A":
        criterion.pop("Ultimate Bushing Bearing Load")
        criticalMode = min(criterion, key=criterion.get)
        designStrengthUnderAxialLoad = min(netSectionUltimateLoad, ultimateBearingLoad)
    else:
        criticalMode = min(criterion, key=criterion.get)
...
def exportAxialData(inputDataList, dataList):
    outputData = {
        ...
        "Ultimate Bushing Bearing Load": ["N/A" if len(dataList[4]) == 2 else dataList[4]["Ultimate Bushing Bearing Load"]]
    }
    ...
    inputData = {
        ...
        "Lug Width": [inputDataList[3]],
        "Lug Thickness": [inputDataList[4]],
        "Ultimate Tensile Strength": [inputDataList[5]],
        "Yield Tensile Strength": [inputDataList[7]],
        "Elastic Modulus": [inputDataList[8]], 
        "Ultimate Strain": [inputDataList[9]],
        "Design Load": [inputDataList[10]],
        "Factor Of Safety": inputDataList[11],
        "Yield Bush Bearing Compressive Strength": [inputDataList[6]]
    }
